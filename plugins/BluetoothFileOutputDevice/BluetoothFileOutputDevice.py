...
class BluetoothFileOutputDevice(OutputDevice):
    """Implements an OutputDevice that supports sending to arbitrary bluetooth devices."""

    def __init__(self):
        super().__init__("bluetooth_file")

        self.setName(catalog.i18nc("@item:inmenu", "Bluetooth File Send"))
        self.setShortDescription(catalog.i18nc("@action:button Preceded by 'Ready to'.", "Sending to Device"))
        self.setDescription(catalog.i18nc("@info:tooltip", "Sending to Device"))
    ...
    def _onWriteJobFinished(self, job):
        ...
        if job.getResult():
            self.writeSuccess.emit(self)
            message = Message(catalog.i18nc("@info:status Don't translate the XML tags <filename>!", "Saved to <filename>{0}</filename>").format(job.getFileName()), title = catalog.i18nc("@info:title", "File Saved"))
            message.addAction("send_file", catalog.i18nc("@action:button", "Send File to Blutooth device"), "send-file", catalog.i18nc("@info:tooltip", "Send the output file to bluetooth device"))
            message._folder = os.path.dirname(job.getFileName())
            message.actionTriggered.connect(self._onMessageActionTriggered)
            message.show()
        else:
    ...
    def _onMessageActionTriggered(self, message, action):
        if action == "send_file" and hasattr(message, "_folder"):
            if sys.platform == "win32":
                subprocess.run(['fsquirt.exe', 'test.txt'])
