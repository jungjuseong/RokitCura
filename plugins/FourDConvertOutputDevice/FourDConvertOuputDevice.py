...
class FourDConvertOuputDevice(OutputDevice):
    """Implements an OutputDevice that supports converting to 4d format."""

    def __init__(self):
        super().__init__("four_d_file")

        self.setName(catalog.i18nc("@item:inmenu", "Convert to 4D format"))
        self.setShortDescription(catalog.i18nc("@action:button Preceded by 'Ready to'.", "Convert to 4D"))
        self.setDescription(catalog.i18nc("@info:tooltip", "Convert to 4D format"))
    ...
    def _onWriteJobFinished0(self, job):
        self._writing = False
    ...
    def _onWriteJobFinished(self, job):
        ...
        if job.getResult():
            self.writeSuccess.emit(self)
            message = Message(catalog.i18nc("@info:status Don't translate the XML tags <filename>!", "Saved to <filename>{0}</filename>").format(job.getFileName()), title = catalog.i18nc("@info:title", "File Saved"))
            message.addAction("convert-gcode-to-4d", catalog.i18nc("@action:button", "Convert to 4d format"), "convert-gcode-to-4d", catalog.i18nc("@info:tooltip", "Send the output file to bluetooth device"))
            message._folder = os.path.dirname(job.getFileName())
            message._file_name = job.getFileName()
            message.actionTriggered.connect(self._onMessageActionTriggered)
            message.show()
        else:
            message = Message(catalog.i18nc("@info:status Don't translate the XML tags <filename> or <message>!", "Could not save to <filename>{0}</filename>: <message>{1}</message>").format(job.getFileName(), str(job.getError())), lifetime = 0, title = catalog.i18nc("@info:title", "Warning"))
    ...
    def _onMessageActionTriggered(self, message, action):
        CONVERTER_EXE_FILE = "C:\\Program Files\\Rokit\\convert4d.exe"
        if action == "convert-gcode-to-4d" and hasattr(message, "_folder"):
            message = Message(catalog.i18nc("@info:status", "Converter params <filename>{0}</filename>").format(message._file_name), title = catalog.i18nc("@info:title", "Info"))
            message.show()
            if sys.platform == "win32":
                if  os.path.isfile(CONVERTER_EXE_FILE):
                    subprocess.run([CONVERTER_EXE_FILE, message._folder])
                else:
                    message = Message(catalog.i18nc("@info:status", "Converter not found <filename>{0}</filename>").format(CONVERTER_EXE_FILE), title = catalog.i18nc("@info:title", "Error"))
                    message.show()
