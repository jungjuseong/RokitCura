...
class RokitGCodeConverter:
    def __init__(self) -> None:    
        ...
        self._uv_enable_list = [self._getExtrudersProperty(index,"uv_enable") for index in self._JoinSequence]
    ...
    def _convertOneLayerGCode(self, one_layer_gcode) -> str:        
        ...
        for index, gcode in enumerate(gcode_list):
            ...
            if self._MarlinCodeForRemove.match(gcode):
                ...
                gcode_list[index] = uv_code + self._ExtruderNames[self._current_extruder_index] + self._getExtraExtruderCode()
    ...
    def _getUVLocation(self, index) -> None:
        extruder_x_position = self._Extruder_X_Position[index]
        return self._GCODE['G0_X_Y'] % (extruder_x_position, -50)

    # UV 명령어 삽입
    # dispenser 설정 명령어 삽입
    def _get_UV_Code(self, index) -> str:
        if self._uv_enable_list[index] == False:
            return ''
