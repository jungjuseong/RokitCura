        ...
        for index, lines in enumerate(gcode_list):            
            ...
            for index, command in enumerate(layer_commands):                
                if command.startswith("G1"):
                    layer_commands[index] = command[:command.find("E") - 1]
                    if shotFlag == True:
                        if command.find("F") == -1: # split한후 인자의 갯수로 판단
                            layer_commands[index] = "M301 ;SHOT\n" + layer_commands[index]
                            shotFlag = False
                else if command.startswith("G0") and shotFlag == False:
                    layer_commands[index] = "M330 ;STOP\n" + layer_commands[index]
                    shotFlag =True
                else if command.startswith("T"):
                    layer_commands[index] = "D"+ str(int(command[-1]) + 1) # syringe number

                layer_commands[index] = layer_commands[index].replace("-.","-0.")
