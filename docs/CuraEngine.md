...
`Settings` 클래스는 키와 `Settings` 인스턴스를 가진 매핑이다. 이 인스턴스 각각은 모든 설정 프로퍼티가 들어 있다. 현재 이것은 문자열로 저장된 설정 값, 익스트루더를 가리키는 포인터로 저장된 특정 익스트루더의 제약 사항을 포함한다. 설정 값은 여기에 문자열로 저장되며 나중에 다양한 방식으로 해석될 수 있다. 예를 들어 각도를 나타내는 설정은 어디에서는 `dgree`로 어디에서는 `radian`으로 해석될 수 있다. 설정 값을 요청할 때는 어떤 타입이 리턴되어야 할지를 템플릿으로 지정한다.
...
각 `Settings` 인스턴스는 scene에 있는 한개를 제외하고 부모가 있다. 설정이 현재의 Settings 인스턴스에 있지 않다면 설정을 부모에게 요청한다. `Scene` 인스턴스에 모든 설정이 들어 있다. scene 객체들의 계층도는 `Settings` 인스턴스에서 부모-자식 관계를 반영한다.

* scene은 트리의 루트. 전역 설정이다.
* scene의 mesh 그룹의 부모. per-mesh-group 설정이다. 
* 익스트루더의 부모는 현재의 mesh 그룹이다. 다음번 mesh 그룹을 처리할 때, 익스트루더 train의 부모는 다음번 mesh 그룹으로 바뀐다. 이들 익스트루더들은 per-extruder 설정을 가진다.
* mesh들의 부모는 이들 mesh들을 프린트하는 익스트루더들이다. mesh들은 per-mesh 설정을 가진다.
...
프린터 특성이 특정 익스트루더로 프린트하도록 설정된다면 어떤 설정을 위한 익스트루더가 제한될 것이다. 예를 들어 infill은 두번째 인스트루더로 프린트하도록 설정된다면 모든 infill 설정은 두번째 익스트루더로 계산되어야 한다. 만일 그 익스트루더의 설정이 있다면 먼저 그 익스트루더의 제약을 계산한다. 그 다음에는 익스트루더의 제약은 다시는 계산하지 않는다. 설정을 구하는 최종 알고리듬은 다음과 같다.

1. 현재 `Settings` 인스턴스가 설정 값이 있다면 그것을 리턴한다. 아니면 ...
2. 설정이 특정 익스트루더에 제한된다면 그 익스트루더로부터 얻어온다. 그 다음부터는 더 이상 그 익스트루더에 재약되지 않는다. 아니면 ...
...
각 scence은 여러개의 mesh 그룹과 여러개의 익스트루더가 들어 있다. 이 mesh 그룹들은 한번에 하나씩 처리된다. 각 mesh 그룹마다 같은 익스트루더들이 사용된다. mesh 그룹은 여러개의 mesh들이 들어 있다.
