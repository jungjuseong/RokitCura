...
class SettingVisibilityPresetsModel(QObject):
    ...
    def __init__(self, preferences: Preferences, parent = None) -> None:
        ...
        recommended_item = self.getVisibilityPresetById("recommended")
        if recommended_item is not None:
            basic_visibile_settings = ";".join(recommended_item.settings)
        else:
            Logger.log("w", "Unable to find the recommended visiblity preset.")
            basic_visibile_settings = ""

        self._preferences = preferences

        # Preference to store which preset is currently selected
        self._preferences.addPreference("cura/active_setting_visibility_preset", "recommended")
